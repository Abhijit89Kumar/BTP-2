...
\draw[decorate, decoration={brace, amplitude=5pt, mirror}, thick, green!50!black] ([xshift=0.2cm]b2.north east) -- ([xshift=0.2cm]b3.south east) node[midway, right=0.3cm, font=\scriptsize, text=green!40!black, align=left] {D never\\leaves SRAM};
\end{tikzpicture}
\end{center}
\vspace{-0.3em}
\begin{center}
\highlight{Key insight:} D is transient --- computed, consumed once, discarded.\\
Inspired by FlashAttention [Dao et al., NeurIPS 2022]
\end{center}
\end{frame}
...
\begin{center}
\begin{tikzpicture}
\node[font=\fontsize{36}{40}\selectfont\bfseries, text=accent] at (0,0.6) {49\%};
\node[font=\small, text=gray] at (0,-0.1) {peak GPU reduction};
\node[font=\fontsize{20}{24}\selectfont\bfseries, text=iitkgp] at (0,-0.8) {2.02 $\to$ 1.02\,GB};
\node[font=\small, text=gray] at (0,-1.3) {D matrix eliminated};
\end{tikzpicture}
\end{center}
\end{block}
\end{column}
...
\begin{center}
\begin{tikzpicture}
\node[font=\fontsize{36}{40}\selectfont\bfseries, text=accent] at (0,0.6) {75\%};
\node[font=\small, text=gray] at (0,-0.1) {traffic reduction};
\node[font=\fontsize{20}{24}\selectfont\bfseries, text=iitkgp] at (0,-0.8) {4.0 $\to$ 1.0\,GB};
\node[font=\small, text=gray] at (0,-1.3) {reads only L, Z tiles};
\end{tikzpicture}
\end{center}
\end{block}
\end{column}
...
\begin{center}
\begin{tikzpicture}
\node[font=\fontsize{36}{40}\selectfont\bfseries, text=accent] at (0,0.6) {40$\times$};
\node[font=\small, text=gray] at (0,-0.1) {speedup over NumPy};
\node[font=\fontsize{20}{24}\selectfont\bfseries, text=iitkgp] at (0,-0.8) {331\,ms};
\node[font=\small, text=gray] at (0,-1.3) {per solve (FP32)};
\end{tikzpicture}
\end{center}
\end{block}
\end{column}
...
Peak GPU Memory & N/A & 2.024\,GB & \highlight{1.024\,GB} \\
TFLOPS & 0.08 & 4.27 & 3.33 \\
\bottomrule
\end{tabular}
\end{center}
\end{frame}
...
The D matrix accounts for \highlight{1.0\,GB} of HBM in PyTorch.\\[0.5em]
The Triton kernel \textbf{completely eliminates} this intermediate $\to$ \highlight{49.4\%} peak memory reduction.
\end{block}
\vspace{0.5em}
\begin{block}{Why It Matters}
\small
\begin{itemize}
    \item Enables larger $N$, $S$ on same hardware
    \item At $N{=}10{,}000$: D would be 40\,GB --- fused approach is the \textbf{only} viable single-GPU solution
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
\begin{tabular}{rrrr}
\toprule
$N$ & \textbf{Speedup} & \textbf{Mem $\downarrow$} \\
\midrule
128  & 1.1$\times$ & 2.9\%  \\
512  & 1.5$\times$ & 9.8\%  \\
1024 & 1.0$\times$ & 16.4\% \\
...
\small
\textbf{Memory savings grow with $N$}. Computation speedup peaks at moderate $N$ (1.5$\times$); at large $N$ the matmul dominates and both backends converge. Extension kernels (more work per tile) achieve clear speedups at all scales.
\end{block}
\end{column}
\end{columns}
\end{frame}
...
\begin{frame}{Grid Search: Optimal $Q^*$}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{gs_qstar_ratio_tractors.png}
\end{center}
\end{column}
\begin{column}{0.48\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{gs_qstar_vs_mu_scatter.png}
\end{center}
\end{column}
\end{columns}
...
\begin{itemize}
    \item \textbf{78\% of products should order below mean demand} (mean $Q^*/\mu = 0.854$)
    \item Aggregate profit improvement: \highlight{+5.3\%} ($\sum\E[\pi^*] = 1.27\text{M}$ vs $\sum\E[\pi_0] = 1.20\text{M}$)
    \item Tractors: $Q^*/\mu = 0.825$; \; Generators: $Q^*/\mu = 0.899$
\end{itemize}
\end{block}
\end{frame}

% ---------------------------------------------------------------------------
\begin{frame}{CVaR Risk Analysis}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{cvar_vs_eprofit_scatter.png}
\end{center}
\end{column}
\begin{column}{0.48\textwidth}
\begin{center}
\includegraphics[width=\textwidth]{cvar_risk_gap_generators.png}
\end{center}
\end{column}
\end{columns}
...
\begin{block}{Key Findings}
\begin{itemize}
    \item Generators carry \highlight{17.5$\times$ more tail risk} than tractors (mean gap 1{,}955 vs 112)
    \item Sanity check $\text{CVaR} \leq \E[\pi]$: \textcolor{goodgreen}{\textbf{PASS}} for all 2,048 products
    \item Risk-averse ordering would shift resources toward lower-variance tractors
\end{itemize}
\end{block}
\end{frame}
